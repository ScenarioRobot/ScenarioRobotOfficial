...
import ScenarioRobot.modules.sql.global_bans_sql as sql
...
from ScenarioRobot.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is part of the Team Scenario\nI can't act against our own."
        )
        return
    ...
    log_message = (
        ...
        f"<b>Event Stamp:</b> <code>{current_time}</code> \nAlert to @userderdead and @i_14344 "
    )
...
def gbanlist(update: Update, context: CallbackContext):
    ...
    with BytesIO(str.encode(banfile)) as output:
        output.name = "scenariogbanlist.txt"
        update.effective_message.reply_document(
            document=output,
            filename="scenariogbanlist.txt",
