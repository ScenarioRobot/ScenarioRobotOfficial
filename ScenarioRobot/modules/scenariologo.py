...
async def lego(event):
 ...
 if event.sender_id == OWNER_ID:
  ...
  if not quew:
     await event.reply('Listen! Provide Some Text To Draw!')
     return
 await event.reply('Creating your logo...wait!')
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./ScenarioRobot/resource/logo/logo.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./ScenarioRobot/resource/Maghrib.ttf", 1000)
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./ScenarioRobot/resource/logo/logo1.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./ScenarioRobot/resource/Chopsic.otf", 330)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
    ...
    fname2 = "Scenario.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="MADE BY @scenariorobot")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @noobxsupport, {e}')
